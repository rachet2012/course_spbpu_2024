...
# Вставил все методы из ноутбука с примерами
...
def calc_pipe(
    ...
    """
    steps = 1000    # Число шагов дискретизации
    step_size = (md_vdp - h0) / steps

    p = [p_wh]
    t = [t_wh]

    # Решаем методом конечных разностей

    for h in np.linspace(h0, md_vdp, steps):
...
def calc_p_wf(
    ...
    """
    p, _ = calc_pipe(p_wh, t_wh, h0, md_vdp, temp_grad, gamma_wat, angle, q_ms, d_tub, roughness)

    return p[-1]


file = open(r'D:\Code\Univer\course_spbpu_2024\homeworks\homework_1\Konstantinov\8.json')
data = json.load(file)
...
result = {'q_liq': list(Q), 'p_wf': list(pressure)}

with open(r'D:\Code\Univer\course_spbpu_2024\homeworks\homework_1\Konstantinov\output.json', 'w') as fp:
